<template>
  <div class="popup">
    <ListItem class="permission-row">
      <CheckBox
        :value="!list.address"
        @click.native="changePermission('address', !list.address)"
        prevent
      />
      <p :class="!list.address ? 'checked' : ''">{{ $t('pages.permissions.login') }}</p>
    </ListItem>
    <ListItem class="permission-row">
      <CheckBox
        :value="!list.messageSign"
        @click.native="changePermission('messageSign', !list.messageSign)"
        prevent
      />
      <p :class="!list.messageSign ? 'checked' : ''">
    ...
    <ListItem class="permission-row">
      ...
      <Input
        :value="list.transactionSignLimit || ''"
        :error="error"
    ...
    <ListItem class="permission-row">
      ...
      <span class="tokens">
        {{ (list.transactionSignLimit - limitLeft).toFixed(2) }}
        <span class="secondary-text">{{ `&nbsp;${$t('pages.appVUE.aeid')}` }}</span>
      </span>
      <span class="currency">{{
        `(${formatCurrency(convertToCurrency(list.transactionSignLimit - limitLeft))})`
      }}</span>
    </ListItem>
    <ListItem class="permission-row">
...
export default {
  ...
  async mounted() {
    ...
    this.limitLeft =
      storageLimitLeft === undefined ? this.list.transactionSignLimit : storageLimitLeft;
  },
  methods: {
